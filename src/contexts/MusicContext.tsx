...
export const MusicProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const savedQueue = sessionStorage.getItem('speedify-queue');
    if (savedQueue) {
      const { track, queue, index } = JSON.parse(savedQueue);
  ...
  useEffect(() => {
    if (currentTrack && currentQueue.length > 0) {
      sessionStorage.setItem('speedify-queue', JSON.stringify({
        track: currentTrack,
        queue: currentQueue,
        index: currentIndex,
      }));
