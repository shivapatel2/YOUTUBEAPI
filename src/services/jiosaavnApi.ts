...
export const convertJioSaavnToTrack = (saavnTrack: JioSaavnTrack): MusicTrack => {
  ...
  const durationInSeconds = parseInt(saavnTrack.duration, 10);

  return {
    id: `jiosaavn_${saavnTrack.id}`,
    title: saavnTrack.name || 'Unknown Title',
    artist: saavnTrack.artists?.primary?.[0]?.name || 'Unknown Artist',
    album: saavnTrack.album?.name || 'Unknown Album',
    duration: isNaN(durationInSeconds) ? 0 : durationInSeconds,
    ...
    source: 'jiosaavn',
    license: 'JioSaavn',
    genre: saavnTrack.language || 'Unknown Genre',
  };
};
